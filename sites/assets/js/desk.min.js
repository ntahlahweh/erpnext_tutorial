var Rollup = (function (exports) {
	...
	*/
	...
	frappe.msgprint = function(msg, title) {
		...
		if(!msg_dialog$1) {
			msg_dialog$1 = new frappe.ui.Dialog({
				title: __("Message"),
				onhide: function() {
					if(msg_dialog$1.custom_onhide) {
						msg_dialog$1.custom_onhide();
					}
					msg_dialog$1.msg_area.empty();
				}
			});
			msg_dialog$1.msg_area = $('<div class="msgprint">')
				.appendTo(msg_dialog$1.body);

			msg_dialog$1.loading_indicator = $('<div class="loading-indicator text-center" \
				style="margin: 15px;">\
				<img src="/assets/frappe/images/ui/ajax-loader.gif"></div>')
				.appendTo(msg_dialog$1.body);

			msg_dialog$1.clear = function() {
				msg_dialog$1.msg_area.empty();
			};

			msg_dialog$1.indicator = msg_dialog$1.header.find('.indicator');
		}
		...
		if(data.clear) {
			...
			msg_exists = msg_dialog$1.msg_area.html();
		}

		if(data.title || !msg_exists) {
			// set title only if it is explicitly given
			// and no existing title exists
			msg_dialog$1.set_title(data.title || __('Message'));
		}

		// show / hide indicator
		if(data.indicator) {
			msg_dialog$1.indicator.removeClass().addClass('indicator ' + data.indicator);
		} else {
			msg_dialog$1.indicator.removeClass().addClass('hidden');
		}

		if(msg_exists) {
			msg_dialog$1.msg_area.append("<hr>");
		// append a <hr> if another msg already exists
		}

		msg_dialog$1.msg_area.append(data.message);
		msg_dialog$1.loading_indicator.addClass("hide");

		msg_dialog$1.show_loading = function() {
			msg_dialog$1.loading_indicator.removeClass("hide");
		};

		// make msgprint always appear on top
		msg_dialog$1.$wrapper.css("z-index", 2000);
		msg_dialog$1.show();
	...
	frappe.hide_msgprint = function(instant) {
		// clear msgprint
		if(msg_dialog$1 && msg_dialog$1.msg_area) {
			msg_dialog$1.msg_area.empty();
		}
		if(msg_dialog$1 && msg_dialog$1.$wrapper.is(":visible")) {
			if(instant) {
				msg_dialog$1.$wrapper.removeClass("fade");
			}
			msg_dialog$1.hide();
			if(instant) {
				msg_dialog$1.$wrapper.addClass("fade");
			}
		}
	};
	...
	frappe.update_msgprint = function(html) {
		if(!msg_dialog$1 || (msg_dialog$1 && !msg_dialog$1.$wrapper.is(":visible"))) {
			frappe.msgprint(html);
		} else {
			msg_dialog$1.msg_area.html(html);
		}
	};
		...
		$(window).on('keydown', function(e) {
			...
			if(frappe.ui.keys.handlers[key]) {
				...
				for(var i=0, l = frappe.ui.keys.handlers[key].length; i<l; i++) {
					...
					var _out = handler.apply(this$1, [e]);
					if(_out===false) {
						out = _out;
					}
				}
	...
	function is_null$1(v) {
		if(v===null || v===undefined || cstr$1(v).trim()==="") { return true; }
	}
	...
	function has_words$1(list, item) {
		if(!item) { return true; }
	...
	function has_common$1(list1, list2) {
	...
	Object.assign(window, {
		...
		toTitle: toTitle,
		is_null: is_null$1,
		set_value_in: set_value_in,
		copy_dict: copy_dict$1,
		validate_email: validate_email,
		validate_spl_chars: validate_spl_chars,
		cstr: cstr$1,
		nth: nth,
		esc_quotes: esc_quotes,
		has_words: has_words$1,
		has_common: has_common$1,
		add_lists: add_lists,
		docstring: docstring,
	...
	function $c$1(command, args, callback, error, no_spinner, freeze_msg, btn) {
	...
	Object.assign(window, {
		$c: $c$1, $c_obj: $c_obj, $c_obj_csv: $c_obj_csv
	});
	...
	frappe.ui.form.MultiSelectDialog = Class.extend({
		...
		make: function() {
			...
			if($.isArray(this.setters)) {
				for (var i = 0, list = this$1.setters; i < list.length; i += 1) {
					var df = list[i];
		...
		with_doctype: function(doctype, callback, async) {
			if(locals.DocType[doctype]) {
				...
				if(localStorage["_doctype:" + doctype]) {
					...
					var cached_doc$1 = cached_docs.filter(function (doc) { return doc.name === doctype; })[0];
					if(cached_doc$1) {
						cached_timestamp = cached_doc$1.modified;
					}
				}
				return frappe.call({
	...
	frappe.db = {
		...
		get_single_value: function (doctype, field) {
			return new Promise(function (resolve) {
				frappe.call('frappe.client.get_single_value', { doctype: doctype, field: field })
		...
		get_doc: function(doctype, name, filters) {
			...
			return new Promise(function (resolve, reject) {
				frappe.call({
					...
					callback: function (r) { return resolve(r.message); }
				}).fail(reject);
			});
		},
		insert: function(doc) {
			return new Promise(function (resolve) {
				frappe.call('frappe.client.insert', { doc: doc }, function (r) { return resolve(r.message); });
			});
		},
		...
		count: function(doctype, args) {
			...
			return new Promise(function (resolve) {
				frappe.call(
					...
					Object.assign(args, { doctype: doctype }),
					function (r) { return resolve(r.message); }
				);
			});
		}
	};
	...
	$.extend(frappe.perm, {
		...
		get_allowed_docs_for_doctype: function (user_permissions, doctype) {
			...
			}).map(function (perm) { return perm.doc; });
		}
	});
	...
	Object.assign(frappe.utils, {
		...
		get_form_link: function(doctype, name, html) {
	  ...
	  this._dispatch = function dispatch (evtName, text, options, globals) {
	    ...
	    if (listeners.hasOwnProperty(evtName)) {
	      for (var ei = 0; ei < listeners[evtName].length; ++ei) {
	        var nText = listeners[evtName][ei](evtName, text, this$1, options, globals);
	...
	window.format_number = function (v, format, decimals) {
		if (!format) {
			...
			if (decimals == null) { decimals = cint$1(frappe.defaults.get_default("float_precision")) || 3; }
		}
	...
	frappe.upload = {
		make: function(opts) {
			...
			// dropzone upload
			var $dropzone = $('<div style="padding: 20px 10px 0px 10px;"/>');
			new frappe.ui.DropZone($dropzone, {
				drop: function (files) {
					$dropzone.hide();

					opts.files = opts.files ? opts.files.concat( files) : files;
			...
			$file_input.on("change", function() {
				if (this.files.length > 0 || opts.files) {
					...
					file_array = file_array.map(
						function (file) { return Object.assign(file, {is_private: opts.is_private ? 1 : 0}); }
					);
					...
					var file_pills = file_array.map(
						function (file) { return frappe.upload.make_file_row(file, opts); }
					);
		...
		make_file_row: function(file, ref) {
			if ( ref === void 0 ) ref = {};
			var show_private = ref.show_private;

			var safe_file_name = frappe.utils.xss_sanitise(file.name);
		...
		read_file: function(fileobj, args, opts) {
			...
			var upload_through_socketio = function() {
				frappe.socketio.uploader.start({
					...
					on_progress: function (percent_complete) {
	...
	frappe.ui.toolbar.Toolbar = Class.extend({
		...
		setup_modules_dialog: function setup_modules_dialog() {
			var this$1 = this;

			this.modules_select = new frappe.ui.toolbar.ModulesSelect();
			$('.navbar-set-desktop-icons').on('click', function () {
				this$1.modules_select.show();
			});
		},
		...
		setup() {
				...
				fields: [
					{
						...
						default: 'user',
						onchange: function () {
							var field = this$1.$setup_for;
							if(field.get_value() === 'everyone') {
								this$1.$user.$wrapper.hide();
								this$1.user = undefined;
								field.set_description(__('Limit icon choices for all users.'));
							} else {
								this$1.$user.$wrapper.show();
								this$1.user = this$1.$user.get_value();
								field.set_description('');
							}
							this$1.$icons_list.refresh();
						}
					},
					{ fieldtype: 'Column Break' },
					{
						...
						onchange: function () {
							this$1.user = this$1.get_value() || frappe.boot.user.name;
							this$1.$icons_list.refresh();
						}
					},
					{ fieldtype: 'Section Break' },
						...
						get_data: function () {
							return new Promise(function (resolve) {
								frappe.call({
									...
									args: {user: this$1.user},
									freeze: true,
									callback: function (r) {
										var icons = r.message.icons;
										var user = r.message.user;
										resolve(icons
											.map(function (icon) {
												var uncheck = user ? icon.hidden : icon.blocked;
			...
			this.dialog.set_primary_action(__('Save'), function () {
				frappe.call({
					...
					args: {
						hidden_list: this$1.$icons_list.get_unchecked_options(),
						user: this$1.user
					},
					freeze: true,
					callback: function () {
			...
			if (frappe.sys_defaults.email_user_password){
				...
				for (var u in email_list) {
					if (email_list[u]===frappe.user.name){
						this$1.set_password(email_list[u]);
					}
				}
			}
