(function () {
	...
	frappe.ui.Filter = class {
		...
		set_field(doctype, fieldname, fieldtype, condition) {
			...
			var cur = {};
			if(this.field) { for(var k in this$1.field.df) { cur[k] = this$1.field.df[k]; } }
		...
		hide_invalid_conditions(fieldtype, original_type) {
			...
			for (var i = 0, list = this$1.conditions; i < list.length; i += 1) {
				var condition = list[i];

				this$1.filter_edit_area.find((".condition option[value=\"" + (condition[0]) + "\"]")).toggle(
	...
	frappe.ui.FieldSelect = Class.extend({
		...
		init: function init(opts) {
			...
			if(this.filter_fields) {
				for(var i in this$1.filter_fields)
					{ this$1.add_field_option(this$1.filter_fields[i]); }
			} else {
				this.build_options();
			}
	...
	frappe.views.BaseList = class BaseList {
		...
		show() {
			...
			frappe.run_serially([
				...
				function () { return frappe.route_options = null; }
			]);
		}
		...
		set_menu_items() {
			...
			this.menu_items.map(function (item) {
				...
				var $item = this$1.page.add_menu_item(item.label, item.action, item.standard);
				if (item.class) {
	...
	frappe.views.ListView = class ListView extends frappe.views.BaseList {
		...
		set_actions_menu_items() {
			...
			this.actions_menu_items.map(function (item) {
		...
		render() {
			...
			if (this.data.length > 0) {
				// append rows
				this.$result.append(
					this.data.map(function (doc) { return this$1.get_list_row_html(doc); }).join('')
				);
			}
			this.on_row_checked();
			this.render_count();
		...
		get_menu_items() {
			...
			if (frappe.user_roles.includes('System Manager')) {
				...
				items.push({
					label: __('Customize'),
					action: function () {
						if(this$1.meta && !this$1.meta.custom) {
			...
			items.push({
				...
				action: function () { return frappe.add_to_desktop(doctype, doctype); },
				standard: true
			});
		...
		parse_filters_from_route_options() {
			...
			for (var field in frappe.route_options) {
				...
				if (!doctype) {
					doctype = frappe.meta.get_doctype_for_field(this$1.doctype, field);
				}
	...
	frappe.views.ListSidebar = class ListSidebar {
		...
		setup_assigned_to_me() {
			...
			this.page.sidebar.find(".assigned-to-me a").on("click", function () {
				this$1.list_view.filter_area.add(this$1.list_view.doctype, "_assign", "like", ("%" + (frappe.session.user) + "%"));
			});
		}
	...
	frappe.views.KanbanView = class KanbanView extends frappe.views.ListView {
		...
		get_card_meta() {
			...
			if (mandatory.some(function (df) { return df.fieldtype === 'Table'; }) || mandatory.length > 1) {
				quick_entry = true;
			}
	...
	frappe.views.FileView = class FileView extends frappe.views.ListView {
		...
		make_new_doc() {
			...
			frappe.ui.get_upload_dialog({
				...
				callback:function () { return this$1.refresh(); }
			});
		}
