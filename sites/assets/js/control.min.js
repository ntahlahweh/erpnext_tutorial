(function () {
	...
	// ignored /home/ntah/frappe-bench/apps/frappe/frappe/public/js/frappe/form/controls/quill-mention/quill.mention.css
	...
	class Mention {
	  constructor(quill$$1, options) {
	    ...
	    quill$$1.on('text-change', this.onTextChange.bind(this));
	    quill$$1.on('selection-change', this.onSelectionChange.bind(this));

	    quill$$1.keyboard.addBinding({
	      key: Keys.TAB,
	    }, this.selectHandler.bind(this));
	    quill$$1.keyboard.bindings[Keys.TAB].unshift(quill$$1.keyboard.bindings[Keys.TAB].pop());

	    quill$$1.keyboard.addBinding({
	      key: Keys.ENTER,
	    }, this.selectHandler.bind(this));
	    quill$$1.keyboard.bindings[Keys.ENTER].unshift(quill$$1.keyboard.bindings[Keys.ENTER].pop());
	  ...
	  highlightItem(scrollItemInView) {
	    ...
	    for (var i = 0; i < this.mentionList.childNodes.length; i += 1) {
	      this$1.mentionList.childNodes[i].classList.remove('selected');
	    }
	  ...
	  renderList(mentionChar, data, searchTerm) {
	    ...
	    if (data && data.length > 0) {
	      ...
	      for (var i = 0; i < data.length; i += 1) {
	        ...
	        li.innerHTML = this$1.options.renderItem(data[i], searchTerm);
	        li.onmouseenter = this$1.onItemMouseEnter.bind(this$1);
	        li.onclick = this$1.onItemClick.bind(this$1);
	        this$1.mentionList.appendChild(li);
	      }
	      this.itemIndex = 0;
	      this.highlightItem();
	...
	frappe.ui.form.ControlComment = frappe.ui.form.ControlTextEditor.extend({
		...
		get_mention_options: function get_mention_options() {
			...
			var at_values = this.mentions.map(function (value, i) {
				return {
					id: i,
					value: value
				};
	...
	class GridRowForm {
		...
		render() {
			...
			for(var fieldname in (this$1.row.grid.fieldinfo || {})) {
				var fi = this$1.row.grid.fieldinfo[fieldname];
	...
	class GridRow {
		...
		setup_columns() {
			...
			for(var ci in this$1.grid.visible_columns) {
				var df = this$1.grid.visible_columns[ci][0],
					colsize = this$1.grid.visible_columns[ci][1],
					txt = this$1.doc ?
				...
				if(!this$1.columns[df.fieldname]) {
					var column = this$1.make_column(df, colsize, txt, ci);
				} else {
					var column = this$1.columns[df.fieldname];
	...
	class Grid {
		...
		refresh(force) {
			...
			for(var ri=0; ri < data.length; ri++) {
				...
				if(this$1.grid_rows[ri]) {
					var grid_row = this$1.grid_rows[ri];
					grid_row.doc = d;
					grid_row.refresh();
				} else {
					var grid_row = new GridRow({
						parent: $rows,
						parent_df: this$1.df,
						docfields: this$1.docfields,
						doc: d,
						frm: this$1.frm,
						grid: this$1
					});
					this$1.grid_rows.push(grid_row);
				}

				this$1.grid_rows_by_docname[d.name] = grid_row;
			}
		...
		truncate_rows(data) {
			...
			if(this.grid_rows.length > data.length) {
				...
				for(var i=data.length; i < this.grid_rows.length; i++) {
					var grid_row = this$1.grid_rows[i];
		...
		set_column_disp(fieldname, show) {
			...
			if($.isArray(fieldname)) {
				...
				for(var i=0, l=fieldname.length; i<l; i++) {
					...
					this$1.set_editable_grid_column_disp(fname, show);
				}
			} else {
		...
		setup_visible_columns() {
			...
			for(var ci in fields) {
				...
				df = this$1.fields_map[_df.fieldname];

				if(!df.hidden
					&& (this$1.editable_fields || df.in_list_view)
					&& (this$1.frm && this$1.frm.get_perm(df.permlevel, "read") || !this$1.frm)
					...
					if(total_colsize > 11)
						{ return false; }
					this$1.visible_columns.push([df, df.colsize]);
				}
			}
			...
			while(total_colsize < 11 && passes < 12) {
				for(var i in this$1.visible_columns) {
					var df = this$1.visible_columns[i][0];
					var colsize = this$1.visible_columns[i][1];
	...
	frappe.ui.form.ControlHTMLEditor = frappe.ui.form.ControlCode.extend({
		...
		make_preview_container: function make_preview_container() {
			var this$1 = this;
			...
			this.$input.on('change keyup paste', frappe.utils.debounce(function () {
				this$1.build_preview();
			}, 300));
		},
		...
		build_preview: function build_preview() {
			if (this.get_value() == '') {
		...
		_createClass(CODE39, [{
			...
			value: function encode() {
				...
				for (var i = 0; i < this.data.length; i++) {
					result += getEncoding(this$1.data[i]) + "0";
				}
		...
		_createClass(ITF14, [{
			...
			value: function encode() {
				...
				for (var i = 0; i < 14; i += 2) {
					result += this$1.calculatePair(this$1.data.substr(i, 2));
				}
	...
	var index$3 = unwrapExports(ITF14_1);
	var ITF14_2 = ITF14_1.ITF14;

	var ITF14 = /*#__PURE__*/Object.freeze({
		default: index$3,
	...
	var index$4 = unwrapExports(ITF_1);
	var ITF_2 = ITF_1.ITF;

	var ITF = /*#__PURE__*/Object.freeze({
		default: index$4,
		...
		_createClass(MSI, [{
			...
			value: function encode() {
				...
				for (var i = 0; i < this.data.length; i++) {
					...
					var digit = parseInt(this$1.data[i]);
	...
	var index$5 = unwrapExports(MSI$2);
	...
	var index$6 = unwrapExports(pharmacode_1);
	...
	var codabar = function (_Barcode) {
		...
		_createClass(codabar, [{
			...
			value: function encode() {
				...
				for (var i = 0; i < this.data.length; i++) {
					result.push(encodings[this$1.data.charAt(i)]);
					// for all characters except the last, append a narrow-space ("0")
					if (i !== this$1.data.length - 1) {
						result.push("0");
					}
				}
				return {
	...
	var index$7 = unwrapExports(codabar_1);
	...
	var index$8 = unwrapExports(GenericBarcode_1);
	...
	var _ITF = ( ITF14 && index$3 ) || ITF14;

	var _ITF2 = ( ITF && index$4 ) || ITF;

	var _MSI = ( MSI$3 && index$5 ) || MSI$3;

	var _pharmacode = ( pharmacode && index$6 ) || pharmacode;

	var _codabar = ( codabar && index$7 ) || codabar;

	var _GenericBarcode = ( GenericBarcode && index$8 ) || GenericBarcode;
	...
	var index$9 = unwrapExports(barcodes);
		...
		_createClass(CanvasRenderer, [{
			...
			value: function render() {
				...
				for (var i = 0; i < this.encodings.length; i++) {
					var encodingOptions = (0, _merge2.default)(this$1.options, this$1.encodings[i].options);

					this$1.drawCanvasBarcode(encodingOptions, this$1.encodings[i]);
					this$1.drawCanvasText(encodingOptions, this$1.encodings[i]);

					this$1.moveCanvasDrawing(this$1.encodings[i]);
				}
	...
	var SVGRenderer = function () {
		...
		_createClass(SVGRenderer, [{
			...
			value: function render() {
				...
				this.prepareSVG();
				for (var i = 0; i < this.encodings.length; i++) {
					var encoding = this$1.encodings[i];
					var encodingOptions = (0, _merge2.default)(this$1.options, encoding.options);

					var group = this$1.createGroup(currentX, encodingOptions.marginTop, this$1.svg);

					this$1.setGroupOptions(group, encodingOptions);

					this$1.drawSvgBarcode(group, encodingOptions, encoding);
					this$1.drawSVGText(group, encodingOptions, encoding);
			...
			value: function prepareSVG() {
				...
				while (this.svg.firstChild) {
					this$1.svg.removeChild(this$1.svg.firstChild);
				}
			...
			value: function drawSvgBarcode(parent, options, encoding) {
				...
				for (var b = 0; b < binary.length; b++) {
					...
					if (binary[b] === "1") {
						...
						this$1.drawRect(x - options.width * barWidth, yFrom, options.width * barWidth, options.height, parent);
						barWidth = 0;
					}
				}
	...
	var index$a = unwrapExports(renderers);
	...
	var _renderers = ( renderers$1 && index$a ) || renderers$1;
	...
	var _barcodes = ( barcodes$1 && index$9 ) || barcodes$1;
	...
	API.prototype.init = function () {
		...
		var renderProperty;
		for (var i in this$1._renderProperties) {
			renderProperty = this$1._renderProperties[i];
			var options = (0, _merge2.default)(this$1._options, renderProperty.options);

			if (options.format == "auto") {
				options.format = autoSelectBarcode();
			}

			this$1._errorHandler.wrapBarcodeCall(function () {
	...
	API.prototype.render = function () {
		...
		if (Array.isArray(this._renderProperties)) {
			for (var i = 0; i < this._renderProperties.length; i++) {
				render(this$1._renderProperties[i], this$1._encodings, this$1._options);
			}
		} else {
